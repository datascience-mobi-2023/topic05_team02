...
quantile(unlist(prism.achilles), probs = c(0.5,0.10), na.rm= TRUE)


hist(unlist(prism.achilles), breaks = 20, main = "Histogram of prism.achilles", xlab = "gene knock-out scores")
abline(v= 0 , col="red")

pheatmap(prism.achilles.kindney_NA_0)
```
...
# Set the threshold #chunk26 #
prism.achilles_threshold_kidney <- quantile(prism.achilles[prism.achilles != 0], probs = c(0.01), na.rm = TRUE)

# Find data points smaller than the threshold
indices <- which(prism.achilles.kindney_NA_0 < prism.achilles_threshold_kidney, 
                 arr.ind = TRUE)

# Extract the row and column names
row_names <- row.names(prism.achilles.kindney_NA_0)[indices[, 1]]
col_names <- colnames(prism.achilles.kindney_NA_0)[indices[, 2]]

# Print the results
prism.achilles.kidney.high_essential_genes <- data.frame(cl = row_names, gene = col_names, "knock-out score" = prism.achilles.kindney_NA_0[indices])
...
#mean vs. quantile
mean(unlist(prism.exp.kidney_high_effect_drugs_targetgenes[prism.exp.kidney_high_effect_drugs_targetgenes !=0]))
quantile(unlist(prism.exp.kidney_high_effect_drugs_targetgenes[prism.exp.kidney_high_effect_drugs_targetgenes !=0]), probs = 0.5)
...
abline(v = c(
  mean(unlist(prism.exp.kidney_high_effect_drugs_targetgenes[prism.exp.kidney_high_effect_drugs_targetgenes != 0])),
  quantile(unlist(prism.exp.kidney_high_effect_drugs_targetgenes[prism.exp.kidney_high_effect_drugs_targetgenes != 0]), probs = 0.5),
  mean(unlist(prism.exp.kidney_high_effect_drugs_targetgenes)),
  quantile(unlist(prism.exp.kidney_high_effect_drugs_targetgenes), probs = 0.5)
),
col = c("lightblue", "blue", "lightgreen", "darkgreen")
)


#show value for the abline
#text( function, par("usr")[4], labels = paste("Text:", round(Function), 2)), pos = 1)
