...
#### Comparison with knock-out scores 

Shows all kidney cancer gene knock-out-scores which are part of the list of target genes of high inhibiting drugs.

```{r}
#set NA to 0
prism.achilles.kindney_NA_0<-prism.achilles.kindney
prism.achilles.kindney_NA_0[is.na(prism.achilles.kindney_NA_0)] <- 0

# Retrieve genes with matching column names from prism.achilles.kindney_NA_0
prism.achilles.kidney_high_inhibiting_drugs <- prism.achilles.kindney_NA_0[, colnames(prism.achilles.kindney_NA_0) %in% colnames(prism.exp.kidney_high_effect_drugs_targetgenes)]

# Print the matching genes
prism.achilles.kidney_high_inhibiting_drugs


# Set the threshold
prism.achilles_threshold_kidney <- -2

# Find data points smaller than the threshold
indices <- which(prism.achilles.kidney_high_inhibiting_drugs 
                 < prism.achilles_threshold_kidney, 
                 arr.ind = TRUE)

# Extract the row and column names
row_names <- row.names(prism.achilles.kidney_high_inhibiting_drugs)[indices[, 1]]
col_names <- colnames(prism.achilles.kidney_high_inhibiting_drugs)[indices[, 2]]

# Print the results
prism.achilles.kidney.high_essential_genes <- data.frame(Row = row_names, Column = col_names, Value = prism.achilles.kidney_high_inhibiting_drugs[indices])

#
prism.achilles.kidney.high_essential_genes
```
