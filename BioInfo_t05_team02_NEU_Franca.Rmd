...
#PCA_for_prism.exp
```{r}
#Vektor mit Zelllinien pro cancertype für colouring
celllines <- list()
# Loop over the cancertypes vector
for (cancertype in cancertypes) {
  # Subset the prism.cl data frame for the current cancer type
  cancer_df <- rownames(prism.cl[which(prism.cl$disease == cancertype), ])
  
  # Subset the prism.exp data frame based on the row names from the cancer_df
  celllines[[cancertype]] <- cancer_df
  
}
#print cellline numbers for a specific cancer
celllines[["cancertype"]]
...
library(factoextra)
#PCA over the genes in exp
pca_exp_genes = prcomp(t(prism.exp), center = FALSE, scale = FALSE)
pca_cnv_genes= prcomp(t(prism.cnv), center= FALSE, scale= FALSE)
autoplot(pca_cnv,
         data= t(prism.cnv),
         colour= rainbow(27562))
#PCA over the cell lines in exp
pca_exp_celllines=prcomp(prism.exp, center=FALSE, scale=FALSE)
autoplot(pca_exp_celllines,
         data=prism.exp,
         colour= as.factor(rainbow(celllines)))
  #liste kann nicht in Integer umgewandelt werden-> kann ich die Daten anders speichern?
  
#for(cellline in cellines)
  #rainbow(sum(cellline))

#theme(legend.title = element_blank()) +
#guides(colour = guide_legend(title = rownames(prism.exp)))
#geom_text(aes(x = PC1, y = PC2, label = rownames(prism.exp)), size = 3)


length(as.factor(prism.cl[-c(48,131,460,472),]$disease))
#rainbow(23,as.factor(prism.cl[-c(48,131,460,472),]$disease
#identify missing rows in exp that are present in cl
unique(c(setdiff(rownames(prism.cl), rownames(prism.exp)), setdiff(rownames(prism.exp), rownames(prism.cl))))
which(rownames(prism.cl)=="ACH-001024")
