...
Cancer is one of the leading death causes worldwide with a new case incidence of 442.4 per 100,000 people and 10 million deaths per year.(@2023, @2023a)

Therefore it becomes more and more important to find new treatments, but drug development is an expensive and time-taking process which is why new approaches need to be put into place.
Drug repurposing is a very promising way to reduce the investment into finding new treatments as many drugs interact with multiple targets and biological pathways beyond their originally intended purpose.

There are several different computational and experimental approaches to drug repurposing but the overall aim is to identify an unknown effect of a known drug by identifying its mode of action (target,selectivity, affinity,...) Ketoconalzone for example was originally developed for fungal infections and then found to be effective in the Cushing syndrome as well.
Repurposing has different advantages: It is safer, because most of the drugs have already been tested in preclinical studies so the risk of failure due to toxicity for example is lower.
Economizing the preclinical trials also reduces the development time and cost.
Investment becomes more attractive because its return is more likely and quicker.

Experimental approaches are binding assays to identify target interactions and phenotypic screening.

Computational approaches are signature matching (transcriptomics, chemical structure, adverse effect signature, shared targets or pathways), computational molecular docking (binding-site complementarity), genome-wide association studies (genetic variation associated with disease phenotypes), pathway or network mapping (up/down-stream genes might be better targets in the same pathway than the identified gene) and retrospective clinical analysis.
As for every developmental method drug repurposing has a risk of failure and limitations, including regulatory and organizational hurdles, as well as patent considerations.
(@Pushpakom_2018 , @Corsello_2020).

### PRISM screen

In order to generate data several thousand cell lines need to be screened to a large amount of different drugs and drug doses to find high cell growth inhibiting candidates.
To facilitate this approach and reduce the amount of work and time required a new method has been developed, called PRISM (profiling relative inhibition simultaneously in mixtures).
PRISM is a pooled approach where each cell line is labeled with a 24-nucleotide-barcode.
It allows a higher diversity of cell lines which is essential for cancer research and treatment as cancer comes in a lot of different variants, higher number of cell lines screened in reduced time and comparability between the cell lines, as they all grow under identical drug concentrations and culture conditions.

#### Cell lines

The cell lines used for PRISM screening are part of the cancer cell line encyclopedia, where about 1000 cell lines have been genetically characterized.

#### PRISM achilles screen

Achilles is a CRISPR/Cas loss-of-function screen.
Genes are selectively knocked-out to test cell viability/growth.
The combination of achilles-screening and drug sensitivity assays facilitates identification of mechanisms of action.
Identification of cancer-specific gene targets and pathways permits more selective killing of cancerous cells instead of the general cell toxicity of known drugs.
(@Dempster_2019, @2023b)

### This report

The aim in this report was to identify pathways and targets that occur in cancer cell lines, as well as the identification of highly effective drugs by computational drug repurposing approaches.
A special focus was put on kidney cancer cell lines.

## Material and Methods

```{r load_datasets, include=FALSE}
load("C:/Users/User/OneDrive/Dokumente/Studium/Semester 4/Drug repurposing/prism_datasets.rda")
load("C:/Users/User/OneDrive/Dokumente/Studium/Semester 4/Drug repurposing/prsim.cl_cleaned.rda")
```
...
| `prism.snv`      | further information about the genes and mutations (rows)                                                                           |

### Data cleanup

Before working with the data sets, data cleanup was necessary.
Sub-data sets for the 17 kidney cancer cell lines were created and for some areas of application NA-values were set to zero.
The replacement by 0 was chosen because in of the interest in for example highly expressed genes (value\>\>0) or highly cell growth inhibiting/promoting drugs (value\>\>/\<\< 0) and 0 can be seen as the to a "no-effect"- statement.

The rownames of prism.cl were set as cell line names and rows with information which was not further needed were removed to reduce the amount of data.

To get a deeper insight into kidney cancer, a closer look at the corresponding data was required.
For this purpose, sub-datasets from the various prism datasets were extracted and saved as `prism.[...].kidney` .

Furthermore, lists for `prism`, its top variance(\>= 75% quantile ) `prism_var_list` and `prism.exp` were created.
In these, the sub-datasets for all cancer types analyzed, are separately accessible such as for Kidney Cancer: `prism_list[["Kidney Cancer"]]`
...
The `ad.test()` allows conclusions about weather a given data sample follows a particular distribution or not.
...
For correlation analysis, the Spearman method was used, which computes the correlation of the ranks of the values in order to minimize the effect of outliers.
It is based on the covariance between two variables, divided by the product of their standard deviation and assumes a linear relationship.

#### K-means clustering

To identify groups of similar samples the eucledian distance between them in the variable space is calculated.
Close objects cluster together.
It is important to keep in mind that k-means identifies clusters even if there are no clear partitions/ extreme distances between data points.
In this report the elbow method was used as the silhouette method did not provide usable results.
Therefore the sum of pairwise squared distances is computed (wss, =within square sum).
The optimal number of clusters is reached when adding clusters does not remarkably improve the wss-value which can be observed as a knick in the plot for various k (cluster)-values.

#### Principal component analysis

Principal component analysis is used to reduce the dimensionality of a dataframe to variables carrying the highest information content (explaining most of the variance).
As the data used for analysis is already centred and scaled this step can be skipped.
A PCA plot (PC1 against PC2) can help to visualize the data and identify groupings/similarity in the data.
The principle of PCA is to find the rotation that makes the correlation matrix diagonal.

#### Linear regression

Linear regression is used to predict a variable out of known variables.
Three steps are effected: data training (learn regression model from data), testing (on independant datasets) and predicting on new data but in a first analysis does steps are not necessary.
A linear relationship between the variables is assumed.
The regression line is computed by the minimization of the sum of squares of the deviations.
The measure of efficiency is the R\^2 value which is the proportion of variance explained by the model.
The higher the value the better the regression model.
...
The correlation values are distributed as in the histogram.
We had a further look on the genes with values below the 2% quantile and above the 98% quantile of the correlation values.
These 704 genes are the ones with high negative and positive correlation between achilles and expression.
...
Since the effect should be as strongly negative as possible, it was decided, after considering the distribution of prism values, to classify drugs below a threshold of the 2% quantile of prism values as highly effective drugs.
This resulted in 429 unique drugs and 668 different drug-dose combinations.
Those drugs have 463 target genes.
...
The possible target genes, do have a lot of missense mutations as well as silent mutations.
Overall there are four genes with apparently more mutations than the others (ANO1, ATR,BRAF,NOS2).
...
The correlation was computed for each gene once using only the kidney cell line samples and once for the whole data set using the Spearman correlation which is less sensitive to outliers.
...
As the sample size exceeds 5000 it was not possible to perform a Shapiro-Wilks test, so for a first analysis a qqplot was plotted (kidney correlation values against a random normal distribution with `n= sample size` of correlation values and `mean= mean` of correlation values).
...
##### Of highly effective treatments (2% quantile)

In order to find out if a specific drug mode of action and/or target and therefore pathways are overrepresented in cancer which would give a guiding principle for further drug research, k-means clustering was performed on the highly effective drug/dose combinations.
...
First, a data frame containing only the upper 2% quantile of effective treatments was extracted from the prism data frame to reduce it to a limited number of most effective drugs.

```{r prism.treat_subset_high_eff_drugs, echo=FALSE}
#Set threshold depending on the distribution of prism 
...
This allowed the identification of 3 clusters with sizes n(drugs)= 672; 470; 226.
...
Some targets were a lot more represented than others (see section "targets of highly effective drugs") but not important for the clustering.
The often represented disease areas were the same in all clusters and are visualized in the table below.
Interestingly, drugs known to be efficient in infectious diseases are highly represented, as well as oncologic, cardiologic anf hematologic malignancy drugs.
A closer look on the infectious disease drugs showed that most of them are tubulin polymerization inhibitors and other DNA-replication-related mode of actions.
The occurrence of mode of actions can be seen in the dotchart below.
A clustering by mode of action is possible but not clear enough.
Some modes of action are only highly represented in one cluster (eg aurora kinase inhibitor in cluster 1, HSP inhibitor in cluster 2, tubulin polymerization inhibitor in 1&2 but very little in 3 etc.) whereas other modes of action are equally spread beyond the clusters (HDAC inhibitor, ATPase inhibitor,...).
Especially cluster 1 and 2 seem very similar in the distribution of modes of action.
The question on which similarity characteristic the clustering happens remains open but as clusters are identified even if there is no clear structure in the data it is also possible that the highly effective drugs are randomly spread and don't cluster at all.
At last, the drug dose was further investigated but again, the distribution was very similar in all three clusters with peaks at (0-0.249),(0.625-0.749), (2,5-2,624), (10-10,124).
...
##infectious diseases
infectdisease<-prism_cluster_all[prism_cluster_all$disease.area== "infectious disease",]
infectdisease<-infectdisease[!rownames(infectdisease) %in% rownames(infectdisease)[apply(infectdisease, 1, function(x) any(is.na(x)))],]

#specific drugs
table(prism_cluster_all$name)[which(as.numeric(table(prism_cluster_all$name))>5)]

#dose
hist(prism.treat.high_effect_drugs$dose, breaks=seq(0,12, by=0.25), main= "Frequency of drug dose in highly efficient drug/dose combinations", xlab= "dose")
...
# Empty list to store mode of actions
moa_counts_sel <- list()

# Loop through each dataframe
for (df in list(prism.cluster1, prism.cluster2, prism.cluster3)) {
  moa_counts <- table(df$moa)
  moa_counts_sel[[length(moa_counts_sel) + 1]] <- names(moa_counts[moa_counts > 5])
}
#combine moa of the three clusters
combined_moa <- unique(unlist(moa_counts_sel))
prism_cluster_all_often <- prism_cluster_all[prism_cluster_all$moa %in% combined_moa, ]

# Number of unique categories
#length(unique(prism_cluster_all_often$moa))
# Create a color palette using rainbow colors
library(pals)
library(ggplot2)
ggplot(prism_cluster_all_often, aes(x = cluster, fill = moa)) +
  geom_bar() +
...
Principal component analysis was performed in order to find out if there are similarities within cell lines to possibly reduce the dataset and to present the underlying structure of the data.
WHile some cell lines cluster very good together and seem to have a lot of similarities so similar gene expression profiles (e.g. skin, central nervous system, bone), other cell lines have widely spread gene expressions (e.g. ovary, upper aerodigestive).
Kidney cell lines cluster relatively good together.
...
ggplot(plot_wCells.exp, 
  ...
  geom_point() +
  labs(title = "PC1/PC2 biplot of gene expression")
...
ggplot(plot_wCells.cnv, 
       aes(x = PC1, y = PC2, color = lineage)) +
  geom_point()+
  labs(title="PC1/PC2 for copy number variation")
```

Contrarly to the expression principal components, the copy number variation within a cell type is highly dispersed for all cell lines, providing further prove for the missing correlation between copy number variation and gene expression.
...
ggplot(plot_wCells.achilles, 
       aes(x = PC1, y = PC2, color = lineage)) +
  geom_point()+
  labs(title="PC1/PC2 for achilles knockout scores")
```

Principal component analysis for achilles knockout scores revealed that the cell lines of one cell type are relatively widely spread with exception for kidney cell lines, showing a unique close clustering.
...
Nonetheless these interpretations must be done with extreme caution due to the poor fit of the regression model as well as the fact, that tumor suppressor gene expression also underlies regulatory processes within the cell such as micro-RNA's and DNA-methylation @Gregory_2023).
...
## Discussion
During data cleanup removing NA values in the prism dataset was necessary. For further investigation into the dataframe we set NA values to 0 which ultimately led to a small distortion of the data because the actual treatment response remains unknown. Smaller subsets of all dataframes – especially for kidney cancer cell lines - were created to address further research questions. A first look into the main datasets prism, prism.exp and prism.cnv revealed that they were not normally distributed using Anderson-Darling-normality test. This comes not surprising for instance due to the assumed values and in majority none effective drugs in prism or the presence of constitutively active housekeeping genes in prism.exp. (@Janssens_2004)
To identify effective drug targets in kidney cancer a correlation matrix between the achilles knock-out scores as well as the expression data was created. In the most and least 2% quantile of the correlation distribution were 704 genes. Genes with high correlation values can be classified as essential for the cell lines.  The lower 2% quantile of prism.kidney revealed highly effective treatments. The 2% quantile acted as a random threshold which ultimately led to some information loss. Further research revealed 429 different drugs with 668 corresponding drug-dosage combinations for kidney cancer. These treatments had 463 target genes. Overlapping genes between essential genes and those 493 target genes revealed 21 target genes. Mutational analysis revealed that most genes were in majority missense-mutated or silenced. ANO1, ATR, BRAF, BRD4 as well as NOS2 had an highly elevated mutation count. For instance, BRAF – a known oncogene – was mostly missense-mutated, most likely to its constitutively active form. Even tho BRAF-mutations are rare in renal cell carcinoma, the data processing was able to identify BRAF within the 17 kidney cancer cell lines. (@Banerjee_2016) ANO1 is a calcium-dependent chloride transmembrane channel a known biomarker throughout cancer development. For instance, it can help promoting immune escape by modulating the tumor micro-environment. (@Guo_2022). ATR is a known and promising drug target in metastatic clear cell renal cell carcinoma. These cancers are especially resistant to DNA-damaging chemotherapies whereas the DNA damage–sensing kinase ataxia telangiectasia and Rad3-related protein ATR is involved in DNA-repair and damage-sensing. (@Seidel_2022) The bromodomain containing 4 protein BRD4 is also a promising target for impairing renal cancer development. (@Tan_2022) Overall this mutational analysis was able to reveal 4 promising new drug targets which appear to be newcomers in kidney cancer drug development. (@Tan_2022)
Deeper insights into the expression of these 21 target genes were produced by creating a heatmap with the respective expression values for all kidney cancer cell lines. 3 clusters with either a high, modest or low expression through all cell lines were identified. Further investigation in the potential cancerogenic involvement of the other 17 target genes with a low mutational count is needed.
Correlation analysis of CNV and expression for all cell lines and kidney cancer exclusively revealed that they are highly correlated for some genes but not at all or even negatively for others. The distribution of correlation values is widely spread and a Kolgomorov-Smirnov-test showed that the distribution is not normal. The range of kidney-cancer specific correlation values is larger than the overall correlation value range which can be explained by the amount of different cell lines were the extreme values are evened out by the amount of data of total cell lines. To investigate the diverse correlation results a first explorative linear regression was performed with the whole kidney datasets because the main goal was to search for possible overall trends and not generating a model which can explain most or even all expression values with CNV as predictor. The correlation values already proofed otherwise. To create a accurate model more predictor variables are clearly needed which is further confirmed by the R-squared value of 0.01. For the amount of data which the model was able to explain, a higher CNV corresponded to higher expression values. This connection must be looked at with extreme caution and cannot be clearly interpreted. An analogue regression was performed with the knock-out scores as predictor and again expression values as response variable. The same only 11 of the 17 kidney cancer cell lines were present in the achilles dataset so the regression was performed with a smaller dataset. Again, the model was performed to get first insights into a possible relationship between the importance of genes and their expression. The model revealed a R-squared of 0.15 but must also be looked at with extreme caution. Possible higher collinearities between the datasets prohibit the model from being a good predictor for imputing expression values with achilles as a sole predictor variable. Still the higher R-squared value gives a first result in the dependency of expression on other factors were achilles appears to be more important for expression than CNV. CNV is an especially poor predictor for expression due to the fact, that cancer cells adapt to higher degrees of aneuploidy. Transcriptional regulators as well as DNA-Acetylation and -methylatioin and micro-RNAs further regulate gene expression. (@Mohanty_2021) Overall further regression models with more and better predictor models should be found and then subjugated to train-test-splitted datasets in order to assess dependencies of gene expression.
To possibly find well represented drug mode of actions or drug targets , k-means clustering was performed on the 2%-quantile of the most effective drugs for all cell lines and kidney cancer. K-means clustering was performed for K=1:15. The within-cluster-sum revealed k=3 as the best fit. The 3 clusters included 672, 470 and 226 drugs respectively. The disease area, gene target as well as mode of action of the drugs of each cluster were further examined. Some targets appeared to be more represented than others but not in a cluster-specific way. The same can e said for disease area, were there were no clear trends in its distribution between the clusters. Interestingly drugs for infectious diseases are highly represented throughout all clusters – apart from oncologig or heatologic malignancy and cardiologic drugs. The mode of action can also not be clearly divided into the 3 clusters but might show some trends. For instance, aurora kinase inhibitors in one cluster whereas HSP inhibitors are mainly found in the second cluster. Tubulin polymerization ibhibitors can be found in both ot not in the third cluster. ATPase inhibitors or HDAC inhibitors appear to be equally spread between all clusters. Drug dosage clustering was similar and was therefore unable to provide clear results and didn’t produce clear clusters.
To further investigate the high number of drugs previously designated for infectious diseases they were extracted from the clusters and are therefore within the 2% quantile of drug efficacy. A clear majorityof these treatments were microtubule inhibitors or tubulin polymerization inhibitors which is a well-known strategy in cancer growth inhibition in chemotherapy. Well known drugs in this field are e.g. cis-platin or vincristine. (@Joshi_2021) The latter is also tested in the dataset. Other reoccurring mode of actions are DNA replication inhibition. There appears to be no clear trend in indication. To further examine these drugs more research is needed but the results suggest that infectious disease treatments fit especially well for drug repurposing in cancer reseach.