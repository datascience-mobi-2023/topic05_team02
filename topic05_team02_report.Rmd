...
### Drug repurposing
Cancer is one of the leading death causes worldwide with a new case incidence of 442.4 per 100,000 people and 10 million deaths per year.@Electronic{2023,
  month        = jul,
  organization = {WHO},
  title        = {Cancer},
...
There are several different computational and experimental approaches to drug repurposing but the overall aim is to identify an unknown effect of a known drug by identifying its mode of action (target,selectivity, affinity,...)
Ketoconalzone for example was originally developed for fungal infections and then found to be effective in the Cushing syndrome as well.
...
Economizing the preclinical trials also reduces the development time and cost. Investment becomes more attractive because its return is more likely and quicker.
...
As for every developmental method drug repurposing has a risk of failure and limitations, including regulatory and organizational hurdles, as well as patent considerations. (@Pushpakom_2018 , @Corsello_2020).
...
To facilitate this approach and reduce the amount of work and time required a new method has been developed, called PRISM (profiling relative inhibition simultaneously in mixtures). PRISM is a pooled approach where each cell line is labeled with a 24-nucleotide-barcode. It allows a higher diversity of cell lines which is essential for cancer research and treatment as cancer comes in a lot of different variants, higher number of cell lines screened in reduced time and comparability between the cell lines, as they all grow under identical drug concentrations and culture conditions.
...
Achilles is a CRISPR/Cas loss-of-function screen. Genes are selectively knocked-out to test cell viability/growth. The combination of achilles-screening and drug sensitivity assays facilitates identification of mechanisms of action.
...
@Electronic{2023b,
  month        = jul,
  organization = {MolecularCloud},
  title        = {How to perform a CRISPR knockout experiment?},
  url          = {https://www.molecularcloud.org/p/how-to-perform-a-crispr-knockout-experiment-663},
  year         = {2023},
}

### This report
The aim in this report was to identify pathways and targets that occur in cancer cell lines, as well as the identification of highly effecive drugs by computational drug repurposing approaches. A special focus was put on kidney cancer cell lines.
...
load("C:/Users/User/Downloads/cellline_datasets.rda")
load("C:/Users/User/Downloads/prism_datasets.rda")
```
...
Sub-data sets for the 17 kidney cancer cell lines were created and for some areas of application NA-values were set to zero. The replacement by 0 was chosen because in of the interest in for exemple highly expressed genes (value>>0) or highly cell growth inhibiting/promoting drugs (value>>/<< 0) and 0 can be seen as the to a "no-effect"- statement.
...
To get a deeper insight into kidney cancer, a closer look at the corresponding data was required. For this purpose, sub-datasets from the various prism datasets were extracted and saved as `prism.[...].kidney` .
...
For correlation analysis, the Spearman method was used, which computes the correlation of the ranks of the values in order to minimize the effect of outliers. It is based on the covariance between two variables, divided by the product of their standard deviation and assumes a linear relationship.

### k-means clustering, wss
To identify groups of similar samples the eucledian distance between them in the variable space is calculated. Close objects cluster together. It is important to keep in mind that k-means identifies clusters even if there are no clear partitions/ extreme distances between data points. In this report the elbow method was used as the silhouette method did not provide usable results. Therefore the sum of pairwise squared distances is computed (wss, =within square sum). The optimal number of clusters is reached when adding clusters does not remarkably improve the wss-value which can be observed as a knick in the plot for various k (cluster)-values.

### Principal component analysis
Principal component analysis is used to reduce the dimensionality of a dataframe to variables carrying the highest information content (explaining most of the variance). As the data used for analysis is already centred and scaled this step can be skipped. A PCA plot (PC1 against PC2) can help to visualize the data and identify groupings/similarity in the data. The principle of PCA is to find the rotation that makes the correlation matrix diagonal.

### Linear regression
Linear regression is used to predict a variable out of known variables. Three steps are effected: data training (learn regression model from data), testing (on independant datasets) and predicting on new data but in a first analysis does steps are not necessary. A linear relationship between the variables is assumed. The regression line is computed by the minimization of the sum of squares of the deviations. The measure of efficiency is the R^2 value which is the proportion of variance explained by the model. The higher the value the better the regression model.
...
correlation_plot
```
The possible target genes, do have a lot of missense mutations as well as silent mutations. Overall there are four genes with apparently more mutations than the others (ANO1, ATR,BRAF,NOS2).
```{r echo=FALSE}
...
First, a data frame containing only the upper 2% quantile of effective drugs was extracted from the prism data frame to reduce it to a limited number of most effective drugs.
...
Interestingly, drugs known to be efficient in infectious diseases are highly represented, as well as oncologic, cardiologic anf hematologic malignancy drugs. A closer look on the infectious disease drugs showed that most of them are tubulin polymerization inhibitors and other DNA-replication-related mode of actions.
...
The question on which similarity characteristic the clustering happens remains open but as clusters are identified even if there is no clear structur in the data it is also possible that the highly effective drugs are randomly spread and don't cluster at all.
